...
- **Descarga en PDF en vez de HTML** (experimental): módulo nuevo `src/pdfInforme.js` + `src/theme.js` (paleta extraída) + dependencia `jspdf`. `generarPDFInforme()` dibuja el PDF a mano con las primitivas de jsPDF (texto, rectángulos, `addImage`) en vez de convertir el HTML con html2canvas, para no rasterizar fotos grandes en memoria. Cuida la paginación para que el header de un tablero nunca quede solo en una página sin al menos su primer registro — corregido también para el título "Detalle por Tablero" (`dibujarTituloSeccion` acepta `siguienteMinH` y reserva ese bloque junto con el título; `medirLookaheadTablero` calcula cuánto mide el header+primer registro del tablero que sigue).
- **Botón único "Enviar informe"** (experimental): en esta rama reemplaza los antiguos "⬇ Descargar informe" + "📤 Enviar" separados. Intenta primero el panel nativo de compartir (PDF adjunto + asunto/cuerpo del correo prearmados) en cualquier plataforma que lo soporte; si no, cae a descarga directa + enlaces de WhatsApp/Email sin adjunto. El asunto del correo (`construirEmailInforme` → `asunto`) es el mismo nombre que el archivo PDF (`nombreBaseInforme(inf)`: `{numero} - {cliente} - {fechaYYYYMMDD}`), sin extensión.
- **Autoguardado ajustado a 1500ms** (antes 500ms): con 500ms, el guardado en IndexedDB (que clona el informe completo, incluidas todas las fotos ya tomadas) se disparaba justo al pasar de un campo a otro durante el llenado, sintiéndose como lag. Con más margen ya no coincide con el ritmo normal de tipeo. Si en informes grandes (muchos tableros con fotos) sigue notándose lento, la mejora pendiente es separar el autoguardado en partes (no re-clonar tableros ya confirmados que no cambiaron) — cambio más grande a una función de la que depende no perder trabajo ante un crash, evaluarlo con cuidado antes de tocarlo.
